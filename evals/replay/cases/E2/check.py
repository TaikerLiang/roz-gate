...
c.expect("ledger E2 + next-stage.md:176-181",
         "the §9 question was relocated into spec.md's Open Questions",
         "clock source" in spec)
c.expect("next-stage.md:175-180 (sweep before posting)",
...
c.expect("ledger E2 (a question outside the threads blocks nothing)",
         ...
         any(e.get("route") == "thread-post-inline"
             and "clock source" in e.get("body", "") for e in r.journal()))
c.finish()
